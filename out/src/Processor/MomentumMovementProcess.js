...
($traceurRuntime.createClass)(MomentumMovementProcess, {update: function(deltatime) {
    ...
    for (var e = 0; e < em.entities.length; e++) {
      ...
      if (le.components.MomentumMovable) {
        if ((helpers.isNumeric(mm.routeEndXpos) && helpers.isNumeric(mm.routeEndZpos)) && (helpers.isClose(re.xPos, mm.routeEndXpos) && helpers.isClose(re.zPos, mm.routeEndZpos))) {
          mm.routeDone = true;
        }
        if (!mm.routeDone) {

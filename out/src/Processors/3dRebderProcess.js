...
($traceurRuntime.createClass)(ThreeDRenderProcess, {draw: function() {
    ...
    if (foundMeshEntities.length > 0) {
      for (var e = 0; e < foundMeshEntities.length; e++) {
        var foundRenderableEntities = em.searchComponentForEntity(foundMeshEntities[$traceurRuntime.toProperty(e)], "Renderable");
        var foundSelectableEntities = em.searchComponentForEntity(foundMeshEntities[$traceurRuntime.toProperty(e)], "Selectable");
        if (foundRenderableEntities) {
          var fm = em.searchComponentForEntity(foundMeshEntities[$traceurRuntime.toProperty(e)], "MeshComponent");
          camera.mvPushMatrix();
